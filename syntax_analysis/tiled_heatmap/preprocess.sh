...
## Find the intersection with all genes
...
do
	...
	bedtools intersect -u -a ${motif} -b ${WORKDIR}/genes_peaks_subset.bed > ${WORKDIR}/${motif_name}_motif_gene.bed
	...
	do
		...
		echo $tf_name
		cat ${WORKDIR}/${motif_name}_motif_gene.bed | wc -l
		cat ${WORKDIR}/${motif_name}_gene.bed | wc -l
		bedtools intersect -c -a ${WORKDIR}/${motif_name}_motif_gene.bed -b ${WORKDIR}/${motif_name}_gene.bed > ${WORKDIR}/outs/${motif_name}_outs/${tf_name}.bed
	done

	rm ${WORKDIR}/${motif_name}_motif_gene.bed
