...
count <- '/mount/samee/hali_data/aggr_results_whole_heart/agg_ec/outs/filtered_peak_bc_matrix.h5'
meta <- '/mount/samee/hali_data/aggr_results_whole_heart/agg_ec/outs/singlecell_processed.csv'
frag <- '/mount/samee/hali_data/aggr_results_whole_heart/agg_ec/outs/fragments.tsv.gz'
# Load the pre-processed scRNA-seq data
snRNA <- readRDS("/mount/samee/hali_data/aggr_results_whole_heart/agg_ec/outs/ec_integrated_v2.rds")
...
png('ec_before_harmony_label.png', width=800, height=600)
...
# save the object
save(obj,file="ec_before_harmony.Robj")
...
umapCoord <- as.data.frame(Embeddings(object = obj[["umap"]]))
obj@meta.data <- cbind(obj@meta.data, umapCoord)
...
png('ec_harmony_predicted_id.png', width=800, height=600)
DimPlot(object = obj, group.by = 'predicted.id', repel = TRUE, label = TRUE) + NoLegend()
dev.off()

pdf('ec_harmony_predicted_id.pdf')
DimPlot(object = obj, group.by = 'predicted.id', repel = TRUE, label = TRUE) + NoLegend()
dev.off()

png('ec_harmony_dtype.png', width=800, height=600)
DimPlot(object = obj, group.by = 'dtype', repel = TRUE, label = TRUE) + NoLegend()
dev.off()

png('ec_harmony_seurat_clusters.png', width=800, height=600)
